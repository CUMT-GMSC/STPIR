...
class SessionGraph(Module):
    ...
    def m_self(self,ht):
        ...
        # 计算注意力权重
        # 计算注意力权重
        attention_weights = torch.matmul(query, key.transpose(0, 1)) / torch.sqrt(torch.tensor(200.))

        # 将注意力权重经过 softmax 函数归一化
    ...
    def forward(self, inputs, A, alias_inputs, A_hat, D_hat,mask):  # inputs[b,s], A[b,s,s]
        ...
        ht = seq_hidden_gnn[torch.arange(mask.shape[0]).long(), torch.sum(mask, 1) - 1]# batch_size x latent_size

        ht_num = torch.sum(mask, 1) - 1

        weighted_sum = ht
        for i in range(1):
            weighted_sum = self.m_self(weighted_sum)

        for i in range(len(ht_num)):
            mask_hidden = seq_hidden_gnn[i,ht_num[i],:]
            mask_hidden = mask_hidden + weighted_sum[i]
            seq_hidden_gnn[i, ht_num[i], :] = mask_hidden

        seq_hidden_gnn = seq_hidden_gnn
...
def forward(model, i, data):
    ...
    hidden, target_emb, att_hidden, relation_emb = model(items, A, alias_inputs, A_hat, D_hat,mask)
